...
mod tests {
    ...
    pub async fn v4() {
        let pool_key: PoolKey = PoolKey {
            currency0: Address::from_str("0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d")
                .unwrap(),
            currency1: Address::from_str("0xAB3dBcD9B096C3fF76275038bf58eAC10D22C61f")
                .unwrap(),
    ...
    fn test_price_to_tick() {
        let range = 1..10;
        for n in range {
            println!("testing price: {}", U256::from(4295128739u64) << n,);
            let _tick =
                tick_math::tick_from_price(U256::from(4295128739u64) << n).unwrap();
        }
    }
    #[test]
    fn test_tick_to_price() {
        ...
        let mut prev_price = tick_math::price_from_tick(p_tick).unwrap();

        for tick in (min_tick + 1)..max_tick {
            let c_tick = I24::try_from(tick).unwrap();
            let cur_price = tick_math::price_from_tick(c_tick).unwrap();
