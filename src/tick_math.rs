//////////! Utility functions for Uniswap V3 tick jjitmap and tick index math

use alloy::primitives::{
    I256, U256, U512, aliases::I24,
};
...
pub fn word_index(
    ...
    let divisor =
        I24::try_from(256).unwrap(); // Infallible for 256
    let quotient = normalized_tick
        .div_euclid(divisor);
...
pub fn price_from_tick(
    target_tick: I24,
) -> Option<U256> {
    println!(
        "calculating prince fomr tick: {}",
        target_tick
    );
    let max_tick: I24 =
        I24::try_from(887272).unwrap();
    ...
    let shifted = p256 >> 32;
    let sqrt_price_x96_u256 = if p256
        & ((U256::ONE << 32)
            - U256::ONE)
        != U256::ZERO
    {
...
pub fn tick_from_price(
    ...
    let result = if tick_high
        == tick_low
    {
        tick_high
    } else {
        if price_from_tick(tick_high)?
            >= sqrt_price_x96
        {
            tick_high
        } else {
...
pub fn compute_amount_possible(
    ...
    println!(
        "computing amount possible"
    );
    // Q96 = 2^96
    let q96: U512 = U512::ONE << 96;

    //promote everything to U512
    let liq: U512 = U512::from(
        *available_liquidity,
    );
    let cur: U512 =
        U512::from(*current_sqrt_price);
    let nxt: U512 =
        U512::from(*next_sqrt_price);
...
pub fn compute_price_from1(
    ...
    let dy_q96 =
        amount.checked_mul(q96)?;
    // 2) Divide by liquidity:    Δ√P = (Δy·Q96) / L
    let liquidity_u256 = U256::from(
        *available_liquidity,
    );
    let delta_sqrt = dy_q96
        .checked_div(liquidity_u256)?;
    // 3) Apply to current √P
    let cur: U256 =
        U256::from(*current_sqrt_price);
    let next = if add {
...
pub fn update_liquidity(
    ...
    if liquidity_net < 0 {
        ...
        let abs_net = U256::from(
            liquidity_net.abs() as u128,
        ); // Convert abs(i128) to u128 then U256
        current_liquidity
            .checked_sub(abs_net)
    } else {
        // If liquidity_net is positive or zero, it means liquidity is added.
        let pos_net = U256::from(
            liquidity_net as u128,
        ); // Convert positive i128 to u128 then U256
        current_liquidity
            .checked_add(pos_net)
    }
}
            ...
            if from0 {
                        ...
                        .len()
                {
                    return Err(TickError::Overflow(
                        word_index(normalize_tick(current_tick, pool.tick_spacing))).into()
                        );
                } // No ticks above
                i + 1
            } else {
                if i == 0 {
                    return Err(TickError::Underflow(
                        word_index(normalize_tick(current_tick, pool.tick_spacing))).into() 
                    );
                } // No ticks below
                i - 1
        ...
        Err(i) => {
            if from0 {
                if i >= pool
                    ...
                    .len()
                {
                    return Err(TickError::Overflow(word_index(normalize_tick(current_tick, pool.tick_spacing))).into());
                } // No ticks above
                i
            } else {
                if i == 0 {
                    return Err(TickError::Underflow(word_index(normalize_tick(current_tick, pool.tick_spacing))).into());
                } // No ticks below
                i - 1
            }
        }
...
while remaining > U256::ZERO {
    ...
    let next_tick = pool
        ...
        .ok_or(MathError::A)?;
    println!("next_tick: tick={} liquidity_net={:?}", next_tick.tick, next_tick.liquidity_net);

    // calculate the next tick’s price
    let next_price = price_from_tick(next_tick.tick)
        .ok_or(MathError::A)?;
    println!("calculated next_price from tick {}: {}", next_tick.tick, next_price);

    // calculate the current price (based on current_tick)
    let current_price_v = price_from_tick(current_tick)
        .ok_or(MathError::A)?;
    println!("calculated current_price from current_tick {}: {}", current_tick, current_price_v);
       let old_tick_index =
            next_tick_index;
        next_tick_index = if from0 {
            next_tick_index
                ...
                .ok_or(
                    TickError::Overflow(
                       word_index(normalize_tick(current_tick, pool.tick_spacing))
                    ),
                )?
        } else {
            next_tick_index
        ...
        if remaining < possible {
            ...
            let delta = if from0 {
                ...
                u512_curr_liq
                    .checked_mul(
                        inv_curr.checked_sub(inv_new)
                            .ok_or(MathError::A)?).ok_or(MathError::A)?
                    .checked_div(U512::from(1u128 << 96)).ok_or(MathError::A)?
            };
    ...
    pool.liquidity = curr_liq;
    pool.x96price = curr_price;
